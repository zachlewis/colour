...
multi_signal_unpack_data` method.
    ...
    def test_uncertainty(self):
        """
        Tests :func:`colour.continuous.multi_signal.MultiSignal.uncertainty`
        method.
        """

        self.assertAlmostEqual(
            self._multi_signal.uncertainty(0.5), 0.5, places=7)

        np.testing.assert_array_almost_equal(
            self._multi_signal.uncertainty(np.linspace(0, 9, 10) + 0.5),
